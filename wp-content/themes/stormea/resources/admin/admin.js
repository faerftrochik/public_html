...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".stormea-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.stormea_admin) {
    return;
  }

  var _window$stormea_admin = window.stormea_admin,
      builderStatusData = _window$stormea_admin.builderStatusData,
      getStartedData = _window$stormea_admin.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.stormea-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".stormea-admin-big-notice").addClass("processing");
      $(".stormea-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".stormea-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.stormea-admin-big-notice");
    });
  }
  ...
  $notice_root = $notice_container.closest(".stormea-admin-big-notice");
  $custom_close_button = $notice_root.find(".stormea-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".stormea-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.stormea_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
