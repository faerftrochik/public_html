...
function stormea_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'stormea_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function stormea_theme() {
	return Theme::getInstance();
}

function stormea_assets() {
	return stormea_theme()->getAssetsManager();
}
...
stormea_theme()
	...
	->register_menus(
		array(
			'header-menu' => esc_html__( 'Header Menu', 'stormea' ),
			'footer-menu' => esc_html__( 'Footer Menu', 'stormea' ),
		)
	);
...
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	stormea_assets()
		...
		->registerStylesheet( 'stormea-theme', '/theme/theme.css' );
	stormea_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		stormea_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'stormea_print_color_scheme', 0 );
} else {
	stormea_assets()
		->registerStyle( 'stormea-theme', stormea_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function stormea_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'stormea_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'stormea_after_activation_redirect_url' );
function stormea_after_activation_redirect_url( $url ) {
...
function stormea_get_builder_plugin_slug() {
...
function stormea_theme_plugins( $plugins ) {

	$plugin_slug = stormea_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'stormea' ) : __( 'Kubio', 'stormea' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'stormea_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === stormea_get_builder_plugin_slug() ) {
                ...
                <div class="kubio-admin-row get-started-imported notice notice-success">
                    <div>
                        <p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'stormea' ),
									stormea_theme()->getName()
								)
							);
							?>
                        </p>
                        <p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'stormea' ),
									stormea_theme()->getName()
								)
							);
							?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'stormea' ) ); ?>
                        </a>
...
function stormea_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'stormea_try_online_url', 10 );

function stormea_render_header_style() {

	?>
    <base target="_top">
	<?php

	stormea_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'stormea_render_header_style', 100 );
