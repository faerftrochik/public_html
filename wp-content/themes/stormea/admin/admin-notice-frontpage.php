...
wp_localize_script(
	...
	array(
		...
		'builderStatusData' => array(
			'status'                          => stormea_theme()->getPluginsManager()->getPluginState( stormea_get_builder_plugin_slug() ),
			'install_url'                     => stormea_theme()->getPluginsManager()->getInstallLink( stormea_get_builder_plugin_slug() ),
			'activate_url'                    => stormea_theme()->getPluginsManager()->getActivationLink( stormea_get_builder_plugin_slug() ),
			'slug'                            => stormea_get_builder_plugin_slug(),
...
<div class="stormea-admin-big-notice--container">
	<div class="logo-holder">
		<h2><?php echo sprintf(__("Choose one of the %s predefined designs to start with:", 'stormea'), ucfirst(get_stylesheet())); ?></h2>
	</div>
	<div class="content-holder">
		<ul class="predefined-front-pages">
			<?php foreach ( \ColibriWP\Theme\Defaults::get( 'front_page_designs' ) as $stormea_design_index => $design ) : ?>
				<?php
					$stormea_design_selected = $stormea_design_index === 0 ? 'selected' : '';
				...
				<li data-index="<?php echo esc_attr( $design['index'] ); ?>"
					class="<?php echo $stormea_design_selected; ?>">
                    ...
                    <div class="predefined-front-page-card">
						...
						<div class="front-page-design-wrapper">
							<div class="design-preview-image"
								 style="background-image: url(<?php echo esc_attr( stormea_theme()->getAssetsManager()->getBaseURL() . "/images/{$preview_image_name}" ); ?>)"
							></div>
						</div>
